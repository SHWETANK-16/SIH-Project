...
export async function getTransactions(): Promise<TransactionResponse> {
  ...
  if (import.meta.env.VITE_ENABLE_MOCK_DATA !== 'false') return { items: transactions, total: transactions.length };
  // BACKEND TODO: Send server-side filters, sort, and pagination parameters to the transactions API.
  return apiClient.get<TransactionResponse>('/transactions');
}

export async function getTransaction(id: string): Promise<Transaction> {
  await new Promise((resolve) => setTimeout(resolve, 320));
  if (import.meta.env.VITE_ENABLE_MOCK_DATA !== 'false') return transactions.find((item) => item.id === id) ?? transactions[0];
