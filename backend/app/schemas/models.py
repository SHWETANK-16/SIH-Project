...
class ShapDriver(BaseModel):
    """One feature's exact TreeSHAP contribution to a single decision."""
    feature: str
    label: str
    value: float
    contribution: float  # log-odds; signed
    direction: Literal["increases_risk", "decreases_risk"]
    share: float  # normalised magnitude, 0-1 — the friendly number for a UI bar


class ModelInfo(BaseModel):
    # Fields below `implementation` are populated only by XGBoostRiskEngine.
    # They stay None/empty for engines that expose no model attribution, so the
    # contract is unchanged for any other implementation.
...
class ModelMetrics(BaseModel):
    ...
    # Populated only when a trained model artifact is loaded. `measured` is the
    # flag the UI should trust before presenting any of these as real numbers.
    measured: bool = False
    implementation: str = "MOCK"
    ...
    archetype_flag_rate: dict[str, float] = {}
