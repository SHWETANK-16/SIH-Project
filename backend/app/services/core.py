...
class TransactionService:
    ...
    def _result(self,id,a,p,g,f):
        # Carry the model's own attribution into the explanation layer so the
        # narrative is ordered by what the model actually weighted, not by rule
        # order. Absent for engines that expose no SHAP — the explainer degrades
        # to its threshold-based factors.
        a={**a,"shap_top_drivers":p.get("shap_top_drivers",[]),"model_probability":p.get("model_probability"),"model_implementation":p.get("implementation"),"guardrails_applied":p.get("guardrails_applied",[])}
        # Surface the model's own attribution on the response, not just inside the
        # narrative — the frontend needs the numbers to draw contribution bars.
        model_info=ModelInfo(name=p["model_name"],version=p["model_version"],implementation=p.get("implementation","CALIBRATED_ML"),probability=p.get("model_probability"),model_score=p.get("model_score"),rules_score=p.get("rules_score"),model_weight=p.get("model_weight"),decision_threshold=p.get("decision_threshold"),flagged=p.get("model_flagged"),guardrails_applied=p.get("guardrails_applied",[]),top_drivers=[ShapDriver(**d) for d in p.get("shap_top_drivers",[])],shap_method=p.get("shap_method"))
        return RiskResult(entity_id=id,risk_score=a["risk_score"],risk_level=RiskLevel(a["risk_level"]),priority=RiskLevel(a["priority"]),signals=[RiskSignal(name=s,severity=RiskLevel(a["risk_level"]),value="Synthetic signal") for s in a["signals"]],model=model_info,network=NetworkSummary(network_id=g["network_id"],connected_entities=g["connected_entities"],graph_score=g["graph_score"]),explanation=self.explain.explain(a,f,g))
